...
def main():
    st.title("Placement Dashboard for CSE Students")
    ...
    col1.plotly_chart(fig_domain_count_bar, use_container_width=True)
    ...
    fig_package_cgpa_scatter = px.scatter(df_package_cgpa, x='CGPA', y='Package', 
                                          ...
                                          trendline='ols',
                                          color_discrete_sequence=bright_colors)
    col2.plotly_chart(fig_package_cgpa_scatter, use_container_width=True)
    ...
    fig_company_placement_bar_horizontal = px.bar(df_company_placement, y='Company_placed', x='Placement_Count', 
                                                   ...
                                                   orientation='h',
                                                   color_discrete_sequence=bright_colors)
    col3.plotly_chart(fig_company_placement_bar_horizontal, use_container_width=True)
    ...
    fig_cse_placed_unplaced_pie.update_traces(marker=dict(colors=['red', 'green']))
    col4.plotly_chart(fig_cse_placed_unplaced_pie, use_container_width=True)
    ...
    fig_cse_packages_hist = px.histogram(df_cse_packages, x='Package', title='Distribution of Packages for CSE Branch', color_discrete_sequence=['yellow'])
    col5.plotly_chart(fig_cse_packages_hist, use_container_width=True)
    ...
    fig_backlog_cse_package = px.scatter(df_backlog_cse_package, x='Name', y='Package', color='Branch',
                                         ...
                                         hover_name='Name',
                                         color_discrete_sequence=bright_colors)
    col6.plotly_chart(fig_backlog_cse_package, use_container_width=True)
